...
const Dashboard = () => {
    ...
    const getStatIcon = (type: string) => {
        switch (type) {
            case 'reports': return <FileText className="w-6 h-6 text-blue-500" />;
            case 'companies': return <Building2 className="w-6 h-6 text-purple-500" />;
            case 'analyses': return <TrendingUp className="w-6 h-6 text-green-500" />;
    ...
    const getStatBg = (type: string) => {
        switch (type) {
            case 'reports': return "bg-blue-50";
            case 'companies': return "bg-purple-50";
            case 'analyses': return "bg-green-50";
    ...
    const getCompanyStyles = (type: string) => {
        switch (type) {
            case 'fpt': return { symbol: "bg-blue-100 text-blue-600", button: "bg-blue-600 hover:bg-blue-700" };
            case 'vnm': return { symbol: "bg-green-100 text-green-600", button: "bg-green-600 hover:bg-green-700" };
            default: return { symbol: "bg-gray-100 text-gray-600", button: "bg-primary hover:bg-primary-dark" };
        }
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 font-outfit p-4">
                <div className="bg-white p-8 rounded-[32px] shadow-xl text-center max-w-md">
                    ...
                    <Button onClick={() => window.location.reload()} className="w-full bg-primary hover:bg-primary-dark text-white rounded-xl h-12 font-bold">
                        Try Again
                    </Button>
                </div>
            </div>
    ...
    return (
        <div className="flex min-h-screen bg-gray-50/50 font-outfit">
            ...
            <main className="flex-1 ml-64 p-8 md:p-12 max-w-7xl">
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                            {stats.map((stat, index) => (
                                <div key={index} className="bg-white p-8 rounded-[32px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-gray-50 flex flex-col animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                                    <div className={`w-12 h-12 ${getStatBg(stat.iconType)} rounded-2xl flex items-center justify-center mb-6`}>
                        ...
                        <div className="mb-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {companies.map((company, index) => {
                                    ...
                                    return (
                                        <div key={index} className="bg-white p-8 rounded-[32px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-gray-50 animate-fade-in-up" style={{ animationDelay: `${(stats.length + index) * 100}ms` }}>
                                            <div className="flex items-center justify-between mb-2">
                                                <h4 className="text-lg font-bold text-gray-900">{company.name}</h4>
                                                <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${styles.symbol}`}>{company.symbol}</span>
                                            </div>
                ...
                {/* AI Assistant Banner */}
                <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-600 rounded-[32px] p-10 text-white shadow-xl shadow-blue-200 animate-fade-in-up" style={{ animationDelay: '600ms' }}>
                    {/* Decorative accents */}
